...
app.post('/api/v1/remove-background', upload.single('image'), async (req: Request, res) => {
  try {
    if (!req.file)
      return res.status(400).json({ error: 'Please upload an image.', code: 'MISSING_IMAGE' });
    const metadata = await sharp(req.file.buffer).metadata();
    if (!['jpeg', 'png', 'webp'].includes(metadata.format ?? '')) {
      return res
        .status(415)
        .json({ error: 'Only JPG, PNG and WebP images are supported.', code: 'UNSUPPORTED_TYPE' });
    }
    if (
      !metadata.width ||
      !metadata.height ||
      ...
      return res.status(422).json({
        error: `Images must be no larger than ${MAX_IMAGE_SIDE}×${MAX_IMAGE_SIDE}px.`,
        code: 'IMAGE_TOO_LARGE',
      });
    }
    const response = await axios.post(`${processorUrl}/remove`, req.file.buffer, {
      headers: { 'content-type': `image/${metadata.format}` },
      responseType: 'stream',
      timeout: 120_000,
      maxContentLength: MAX_FILE_SIZE * 2,
    ...
    res
      .status(status)
      .json({ error: 'Background removal failed. Please try again.', code: 'PROCESSING_FAILED' });
  }
});
...
app.use(
  (error: unknown, _req: express.Request, res: express.Response, _next: express.NextFunction) => {
    ...
    if (errorCode === 'LIMIT_FILE_SIZE')
      return res
        .status(413)
        .json({ error: 'Image must be 15 MB or smaller.', code: 'FILE_TOO_LARGE' });
